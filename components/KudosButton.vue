...
<template>
  <div class="kudos-wrapper" aria-label="Sección de kudos">
    ...
    >
      ...
      <span v-if="animating" class="kudos-ripple"></span>
    </button>
    ...
    <div v-if="ownKudos > 0" class="kudos-own-indicator">
      <span class="kudos-own-bar-wrap" aria-label="Tus aplausos">
        <span class="kudos-own-bar" :style="{ width: ownKudosPercent + '%' }"></span>
      </span>
...
// KudosButton es 100% cliente — usa localStorage y escribe en Firestore via Client SDK
...
const ownKudosPercent = computed(() => Math.round((ownKudos.value / MAX_KUDOS) * 100))
...
const handleKudos = async () => {
  ...
  try {
    // Llamada a la API route del servidor para actualizar Firestore
    await $fetch(`/api/posts/${props.postId}/kudos`, { method: 'POST' })
  } catch (err) {
...
.kudos-ripple {
  ...
  inset: 0;
  border-radius: 50%;
  background: radial-gradient(circle, rgba(26, 54, 93, 0.15) 0%, transparent 70%);
...
.count-pop-enter-from { opacity: 0; transform: translateY(-8px) scale(0.85); }
.count-pop-leave-to   { opacity: 0; transform: translateY(8px) scale(0.85); }
.kudos-own-indicator {
  display: flex;
  flex-direction: column;
  align-items: center;
