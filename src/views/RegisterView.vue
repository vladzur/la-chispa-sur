<template>
  <div class="register-bg">
    ...
    <div v-if="tokenLoading" class="register-container">
    ...
    <div v-else-if="!tokenValid" class="register-container">
      <div class="register-card register-card--error">
        <div class="error-icon">🔒</div>
        <h1 class="register-title">Enlace inválido</h1>
        <p class="register-subtitle error-reason">{{ tokenError }}</p>
...
import { validateInviteToken, markTokenAsUsed } from '../services/editorService';
...
onMounted(async () => {
  if (!token.value) {
    ...
    tokenError.value = 'No se proporcionó un enlace de invitación válido.';
  ...
  if (!result.valid) {
    tokenError.value = result.reason || 'Enlace inválido.';
  }
  tokenLoading.value = false;
});
...
const handleRegister = async () => {
  ...
  try {
    ...
    // 2. Crear documento en users con role: 'pending'
    ...
    await markTokenAsUsed(token.value, uid);
